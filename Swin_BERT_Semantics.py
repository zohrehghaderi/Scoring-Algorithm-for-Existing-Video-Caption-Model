...
from Prepare_inputs_sos import SOSSwinBert
...
class Swin_BERT_Semantics(pl.LightningModule):
    ...
    def __init__(self, swin_freeze, in_size, hidden_sizes, out_size, max_length,
                 mlp_freeze, drop_bert):
        super(Swin_BERT_Semantics, self).__init__()

        self.max_length = max_length

        # Swin-Encoder
        self.encoder = swin_encoder(device=self.device, drop=0)
        ...
        self.net = MLP(in_size=in_size, hidden_sizes=hidden_sizes, out_size=out_size,
                       dropout_p=0.1, have_last_bn=True)
    ...
    def forward(self, enc_inputs):
        ...
        model_kwargs = {"encoder_hidden_states": encoder_hidden_states, "inputs_embeds": semantics,
